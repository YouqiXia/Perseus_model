...
namespace TimingModel {
    ...
    void PerfectFrontend::ProduceInst() {
        ...
        if (credit_ < issue_num_) {
            pmu_->Monitor(getName(), "backend loss", issue_num_-credit_);
        }
        ...
        pmu_->Monitor(getName(), "total loss", issue_num_-inst_group_ptr->size());
        pmu_->Monitor(getName(), "frontend loss", produce_num+1);
    ...
    void PerfectFrontend::PmuMonitor_() {
        ...
        if (!credit_) {
            pmu_->Monitor(getName(), "backend loss", issue_num_);
        } else {
            pmu_->Monitor(getName(), "frontend loss", issue_num_);
        }
    }
}